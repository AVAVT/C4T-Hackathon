...
public class BotCharacter : ICharacterController
{
  ...
  public string DoHarvesterThink(GameState gameState)
  {
    ...
    if (needComeBack)
    {
      nextNode = MoveToPos(new Vector2(Character.x, Character.y), Character.team == Team.Blue ? GameConfigs.BLUE_BOX_POS : GameConfigs.RED_BOX_POS);
      return DirectionStringExtension.ToDirectionString(new Vector2(nextNode.x, nextNode.y) - new Vector2(Character.x, Character.y));
    }
    else
    {
  ...
  List<Node> GetNeighbourNodes(Node currentNode)
  {
    ...
    {
      if (i == 0) continue;
      if (currentNode.x + i >= 0 && currentNode.x + i < GameConfigs.MAP_WIDTH) neighbours.Add(listNodes[currentNode.x + i][currentNode.y]);
      if (currentNode.y + i >= 0 && currentNode.y + i < GameConfigs.MAP_HEIGHT) neighbours.Add(listNodes[currentNode.x][currentNode.y + i]);
    }
    return neighbours;
  }
  ...
  List<Tile> GetNeighbourTiles(Tile currentTile, GameState gameState)
  {
    ...
    {
      if (i == 0) continue;
      if (currentTile.x + i >= 0 && currentTile.x + i < GameConfigs.MAP_WIDTH) neighbours.Add(gameState.map[currentTile.x + i][currentTile.y]);
      if (currentTile.y + i >= 0 && currentTile.y + i < GameConfigs.MAP_HEIGHT) neighbours.Add(gameState.map[currentTile.x][currentTile.y + i]);
    }
    return neighbours;
  }
}
